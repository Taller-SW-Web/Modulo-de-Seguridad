# SPEC-11 — Gestión de roles y permisos
...
| Código | Requisito |
|---|---|
| RF-11.1 | El sistema debe soportar de forma exclusiva el conjunto cerrado de 6 roles definidos en el contrato, rechazando códigos en inglés o roles no contemplados. |
| RF-11.2 | El sistema debe permitir que un usuario posea múltiples roles asignados simultáneamente. |
| RF-11.3 | Un usuario con rol `ADMIN_SISTEMA` (permiso `rol.asignar`) debe poder asignar un rol (`POST /api/v1/usuarios/{id}/roles`) o revocarlo (`DELETE /api/v1/usuarios/{id}/roles/{rol}`), **de uno en uno**. No se reemplaza la lista entera: dos administradores que editan a la vez no se pisan los cambios. |
| RF-11.4 | El sistema debe calcular los permisos efectivos realizando la unión sin duplicados de todas las capacidades concedidas por cada rol activo del usuario. |
| RF-11.5 | Al cambiar los roles de un usuario, el sistema debe revocar inmediatamente todos sus tokens de refresco vigentes para forzar una nueva autenticación. |
...
los módulos consumidores (SPEC-17), y así un permiso y un scope no se confunden
nunca.

| Permiso | Qué autoriza | Lo exige | Rol |
|---|---|---|---|
| `usuario.ver` | Listar y consultar cualquier cuenta | SPEC-03 | `ADMIN_SISTEMA` |
| `usuario.crear` | Dar de alta vendedores y roles de gestión | SPEC-03 | `ADMIN_SISTEMA` |
| `usuario.desactivar` | Dar de baja una cuenta | SPEC-04 | `ADMIN_SISTEMA` |
| `usuario.reactivar` | Reactivar una cuenta dada de baja | SPEC-04 | `ADMIN_SISTEMA` |
| `rol.asignar` | Asignar y revocar roles | SPEC-11 | `ADMIN_SISTEMA` |
...
### ESC-11.1 Asignación exitosa de un nuevo rol
...
### ESC-11.2 Asignación de rol inexistente o no permitido *(caso borde)*
...
### ESC-11.3 Intento de modificación por un usuario no autorizado *(caso borde de seguridad)*

**Dado** un usuario autenticado con rol `ADMIN_VENTAS`.

**Cuando** intenta invocar `POST /api/v1/usuarios/{id}/roles`.

**Entonces** el sistema responde con código HTTP `403 SCOPE_INSUFICIENTE` y registra el intento de violación de acceso en auditoría (SPEC-12).
...
### ESC-11.7 Permisos de este módulo en el token de un administrador
...
- Los nombres de los permisos deben seguir la estructura `recurso.accion`, en minúsculas y con punto; los dos puntos quedan para los scopes de SPEC-17. Los permisos van en infinitivo (`usuario.crear`) y los eventos en participio (`usuario.creado`), para no confundirlos.
...
| `GET /api/v1/roles` · `GET /api/v1/permisos` | Llena el catálogo que publica SPEC-18 | ⬜ |
